...
import { getAccountRaw, getTestCase, saveResult, upsertAgent } from "@/lib/store";
...
export async function POST(req: NextRequest) {
  ...
  const spec: TestingAgentSpec = {
    name: `${testCase.name} (${account.provider})`,
  ...
  try {
    // Ad-hoc create the agent from the simulation on the provider.
