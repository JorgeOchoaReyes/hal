...
export function snapshotRun(tc: TestCase): RunContext {
  ...
  return structuredClone({
    ...
    testingAgent: { name: tc.scenario.persona.name, persona: tc.scenario.persona, configuration: { structured: tc.scenario.structured } },
    targetAgent: {
...
export function hostedContext(context: RunContext, account: ProviderAccount, agent: HostedTestingAgent,
  ...
  return structuredClone({
    ...
    testingAgent: {
      ...
      persona: agent.spec?.persona, configuration: { model: agent.spec?.model, voice: agent.spec?.voice, structured: agent.spec?.structured }, direction: outboundIsTarget ? "inbound" : "outbound",
